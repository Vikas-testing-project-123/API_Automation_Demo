...
def test_EmpList():
    url = "http://bam.kockpit.in:4001/EmpList"
    excelPath = "C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\TestData\\UserLogin.xlsx"
    testcases = ["All valid parameter", "Blank UserID L1", "Blank UserId", "Blank Level", "All Blank",
                 "All Invalid", "Invalid UserId", "Invalid Level"]
    testcases1 = ["All valid parameter"]
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "EmpList", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\EmpList.json', 'r')  # open the file in read only mode
        ...
        if response.status_code == int(testData[2]):
            write_Excel(excelPath, "EmpList", test, "Pass:- "+response.text)
        else:
            write_Excel(excelPath, "EmpList", test, "Fail-"+response.text)
