...
def test_FYList():
    url = baseUrlHeader.baseURL+"/FYList"
    ...
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "FYList", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\FYList.json', 'r')  # open the file in read only mode
        ...
        if response.status_code == int(testData[1]):
            write_Excel(excelPath, "FYList", test, "Pass:- "+response.text)
        else:
            write_Excel(excelPath, "FYList", test, "Fail-"+response.text)
