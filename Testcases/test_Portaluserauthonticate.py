...
def test_Portaluserauthonticate():
    ...
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "Portaluserauthonticate", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\Portaluserauthonticate.json', 'r')  # open the file in read only mode
        ...
        json_input = update_content(json_input, "Password", testData[1])
        json_input = update_content(json_input, "DeviceId", testData[2])
        json_input = update_content(json_input, "Domain", testData[3])
        ...
        if response.status_code == int(testData[4]):
            write_Excel(excelPath, "Portaluserauthonticate", test, "Pass:- "+response.text)
        else:
            write_Excel(excelPath, "Portaluserauthonticate", test, "Fail-"+response.text)
