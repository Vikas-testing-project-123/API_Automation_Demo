...
def test_SalesOrder():
    url = "http://bam.kockpit.in:4001/SalesOrder"
    ...
    testcases = ["All valid parameter", "All valid parameter L1", "All valid parameter L2",
                 ...
                 "All Invalid", "Invalid UserId", "Invalid Level", "Invalid Type"]
    testcases1 = ["All valid parameter"]
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "SalesOrder", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\SalesOrder.json', 'r')  # open the file in read only mode
        ...
        if response.status_code == int(testData[3]):
            write_Excel(excelPath, "SalesOrder", test, "Pass:- "+response.text)
        else:
            write_Excel(excelPath, "SalesOrder", test, "Fail-"+response.text)
