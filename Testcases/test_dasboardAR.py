...
def test_dashboardAR():
    ...
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "dashboardAR", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\dashboardAR.json', 'r')  # open the file in read only mode
        ...
        if response.status_code == int(testData[2]):
            write_Excel(excelPath, "dashboardAR", test, "Pass:- "+response.text)
        else:
            write_Excel(excelPath, "dashboardAR", test, "Fail-"+response.text)
