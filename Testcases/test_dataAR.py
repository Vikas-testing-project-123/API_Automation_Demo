...
def test_dataAR():
    url = "http://bam.kockpit.in:4001/data/AR"
    ...
    testcases = ["All valid parameter", "All valid parameter L1", "All valid parameter L2", "All valid parameter L3",
                 "Blank UserId", "Blank Level", "Blank Type", "All Blank",
                 "All Invalid", "Invalid UserId", "Invalid Level", "Invalid Type", "Invalid Company Domain"
                 , "Blank Company Domain"]
    testcases1 = ["All valid parameter"]
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "dataAR", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\dataAR.json', 'r')  # open the file in read only mode
        ...
        if response.status_code == int(testData[4]):
            write_Excel(excelPath, "dataAR", test, "Pass:- "+response.text)
        else:
            write_Excel(excelPath, "dataAR", test, "Fail-"+response.text)
