...
def test_dataDashboardAR():
    url = baseUrlHeader.baseURL+"/data/dashboard/AR"
    ...
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "dataDashboardAR", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\dataDashboardAR.json', 'r')  # open the file in read only mode
        ...
        if response.status_code == int(testData[3]):
            ...
            write_Excel(excelPath, "dataDashboardAR", test, "Fail-"+response.text)
