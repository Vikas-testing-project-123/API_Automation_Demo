...
def test_dataDashboardOpenSalesOrder():
    ...
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "dataDashboardOpenSalesOrder", test)
        i = 0
        ...
        if response.status_code == int(testData[3]):
            write_Excel(excelPath, "dataDashboardOpenSalesOrder", test, "Pass:- "+response.text)
        else:
            write_Excel(excelPath, "dataDashboardOpenSalesOrder", test, "Fail-"+response.text)
