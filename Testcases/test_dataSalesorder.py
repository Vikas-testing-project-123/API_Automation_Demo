...
def test_dataSalesOrder():
    url = baseUrlHeader.baseURL+"/data/SalesOrder"
    ...
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "dataSalesOrder", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\dataSalesorder.json', 'r')  # open the file in read only mode
        ...
        if response.status_code == int(testData[4]):
            ...
            write_Excel(excelPath, "dataSalesOrder", test, "Fail-"+response.text)
