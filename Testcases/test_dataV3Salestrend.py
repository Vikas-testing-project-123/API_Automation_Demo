...
def test_dataV3Salestrend():
    url = baseUrlHeader.baseURL+"/data/V3/SalesTrend"
    ...
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "dataV3Salestrend", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\dataV3Salestrend.json', 'r')  # open the file in read only mode
        ...
        if response.status_code == int(testData[3]):
            write_Excel(excelPath, "dataV3Salestrend", test, "Pass:- "+response.text)
        else:
            write_Excel(excelPath, "dataV3Salestrend", test, "Fail-"+response.text)
