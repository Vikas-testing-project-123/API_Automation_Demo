...
def test_runtimeSales():
    url = baseUrlHeader.baseURL+"/session/getsessioninterval"
    ...
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "getsessioninterval", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\getsessioninterval.json', 'r')  # open the file in read only mode
        ...
        if response.status_code == int(testData[1]):
            write_Excel(excelPath, "getsessioninterval", test, "Pass:- "+response.text)
        else:
            write_Excel(excelPath, "getsessioninterval", test, "Fail-"+response.text)
