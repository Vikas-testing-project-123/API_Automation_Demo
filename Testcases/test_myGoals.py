...
def test_myGoals():
    url = baseUrlHeader.baseURL+"/myGoals"
    ...
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "myGoals", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\myGoals.json', 'r')  # open the file in read only mode
        ...
        if response.status_code == int(testData[3]):
            ...
            write_Excel(excelPath, "myGoals", test, "Fail-"+response.text)
