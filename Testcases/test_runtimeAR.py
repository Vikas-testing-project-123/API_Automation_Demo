...
def test_runtimeAR():
    url = baseUrlHeader.baseURL+"/runTime/AR"
    ...
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "runtimeAR", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\runtimeAR.json', 'r')  # open the file in read only mode
        ...
        if response.status_code == int(testData[1]):
            write_Excel(excelPath, "runtimeAR", test, "Pass:- "+response.text)
        else:
            write_Excel(excelPath, "runtimeAR", test, "Fail-"+response.text)
