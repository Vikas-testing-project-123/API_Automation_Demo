...
def test_runtimeSales():
    url = baseUrlHeader.baseURL+"/runTime/Sales"
    ...
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "runtimeSales", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\runtimeSales.json', 'r')  # open the file in read only mode
        ...
        if response.status_code == int(testData[1]):
            write_Excel(excelPath, "runtimeSales", test, "Pass:- "+response.text)
        else:
            write_Excel(excelPath, "runtimeSales", test, "Fail-"+response.text)
