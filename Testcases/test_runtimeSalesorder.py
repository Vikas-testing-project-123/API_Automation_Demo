...
def test_runtimeSalesorder():
    url = baseUrlHeader.baseURL+"/runTime/SalesOrder"
    ...
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "runtimeSalesorder", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\runtimeSalesorder.json', 'r')  # open the file in read only mode
        ...
        if response.status_code == int(testData[1]):
            write_Excel(excelPath, "runtimeSalesorder", test, "Pass:- "+response.text)
        else:
            write_Excel(excelPath, "runtimeSalesorder", test, "Fail-"+response.text)
