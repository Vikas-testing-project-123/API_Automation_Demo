...
def test_sessionSaveSession():
    url = baseUrlHeader.baseURL+"/session/savesession"
    excelPath = "C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\TestData\\UserLogin.xlsx"
    testcases = ["All valid parameter","Blank UserId", "Blank Company Domain", "Blank Company Name", "Blank Offering", "All Blank",
                 "Invalid Company Domain", "Invalid UserId", "Invalid Offering", "All Invalid", "Invalid  Company Name"]
    testcases1 = ["All valid parameter"]
    for test in testcases:
        print("*************************" + test + "******************************")
        testData = Read_Excel(excelPath, "sessionSaveSession", test)
        i = 0
        ...
        file = open('C:\\Users\\TA0134\\PycharmProjects\\API_Testing\\Json_files\\sessionSaveSession.json', 'r')  # open the file in read only mode
        ...
        json_input = update_content(json_input, "CompanyName", testData[2])
        json_input = update_content(json_input, "OfferingCategory", testData[3])
        json_input = update_content(json_input, "Email", testData[4])
        ...
        if response.status_code == int(testData[5]):
            write_Excel(excelPath, "sessionSaveSession", test, "Pass:- "+response.text)
        else:
            write_Excel(excelPath, "sessionSaveSession", test, "Fail-"+response.text)
